...
def main():
    # Load the MNIST data
    train_images = load_mnist_images('./data/handwriting/train-images.idx3-ubyte')
    train_labels = load_mnist_labels('./data/handwriting/train-labels.idx1-ubyte')
    test_images = load_mnist_images('./data/handwriting/t10k-images.idx3-ubyte')
    test_labels = load_mnist_labels('./data/handwriting/t10k-labels.idx1-ubyte')
    ...
    nn = NeuralNetwork([784, 512, 128, 64, 10], Sigmoid(), CrossEntropyLoss(), AdamOptimizer(0.00005))

    # This training takes 10 minutes on RTX 3080.

    # Train the network (you may adjust epochs and batch_size as needed)
    loss, accuracy = nn.train(train_x, train_y.float(), epochs=500, batch_size=128)
    ...
    # Save the model
    nn.save_model("./models/mnist_handwriting_model.pth")
